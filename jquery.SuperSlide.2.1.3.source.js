  ...
  $.fn.slide = function (options) {
    ...
    return this.each(function () {
      ...
      if (options === 'destroy') {
        ...
        clearTimeout(refs.st);
        clearTimeout(refs.mst);
        clearTimeout(refs.rtnST);
        clearTimeout(refs.resizeTimer);
        clearInterval(refs.inter);
      ...
      refs.inter = null; //autoPlay-setInterval
      refs.mst = null; //trigger-setTimeout
      refs.rtnST = null; //returnDefault-setTimeout
      ...
      if (opts.type == "menu") {
        ...
        if (returnDefault) {
          slider.hover(function () {
            ...
            refs.rtnST = setTimeout(resetOn, delayTime);
          });
        }
      ...
      if (returnDefault) //返回默认状态
      {
        slider.hover(function () {
          ...
          refs.rtnST = setTimeout(function () {
              index = defaultIndex;
              if (defaultPlay) {
                doPlay();
              } else {
      ...
      var setInter = function (time) {
        refs.inter = setInterval(function () {
            opp ? index-- : index++;
            doPlay()
          }, !!time ? time : interTime);
      }
      var setMarInter = function (time) {
        refs.inter = setInterval(doPlay, !!time ? time : interTime);
      }
      // 处理mouseOverStop
      ...
      if (opts.vis == "auto" && scroll == 1 && (effect == "left" || effect == "leftLoop")) {
        ...
        $(window).resize(function () {
          ...
          clearTimeout(refs.resizeTimer);
          refs.resizeTimer = setTimeout(orientationChange, 100);
        });
        orientationChange();
      }
